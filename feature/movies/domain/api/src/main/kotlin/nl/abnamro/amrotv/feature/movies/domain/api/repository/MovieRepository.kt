 ...
 *
 * Implementations are expected to apply a network-first strategy: fetch from the remote
 * source, persist to the local cache, and fall back to the cache on network failure.
 * The caller is responsible for triggering retries — there is no automatic recovery.
 */
interface MovieRepository {
     ...
     *
     * @return [Outcome.Success] with the current list, or [Outcome.Error] on failure
     * (may include stale cached data).
     */
