...
internal class MovieRepositoryImpl @Inject constructor(
    ...
    override suspend fun getTrendingMovies(): Outcome<List<Movie>> = try {
        ...
        // Cache fallback is not fully integrated yet; return cached movies only when the cache is not empty.
        val cached = local.getCachedMovies()
        Outcome.Error(e, cached.takeIf { it.isNotEmpty() })
    }

    override suspend fun getMovieDetail(id: Int): Outcome<MovieDetail> = try {
    ...
    override suspend fun getGenres(): Outcome<List<Genre>> = try {
