...
internal class TmdbMovieDataSourceTest {
    ...
    inner class GivenPage1SucceedsPage2Throws {
        ...
        fun setUp() {
            ...
            coEvery { apiService.getTrendingMovies(any(), 2, any()) } throws RuntimeException("page 2 error")
        }
        ...
        inner class WhenGetTrendingMoviesIsCalled {
            ...
            fun pageFailureIsLogged() = runTest {
                ...
                verify(exactly = 1) { logger.log(LogLevel.WARN, "TmdbMovieDataSource", "Failed to fetch page 2, stopping pagination with 1 movies so far", any()) }
            }
        }
    }
    ...
    inner class GivenPage2Of3Throws {
        ...
        fun setUp() {
            coEvery { apiService.getTrendingMovies(any(), 1, any()) } returns
                TrendingMoviesResponseDto(results = listOf(Dtos.movieDto.copy(id = 1)), page = 1, totalPages = 3)
            coEvery { apiService.getTrendingMovies(any(), 2, any()) } throws RuntimeException("page 2 error")
        }
    ...
    inner class GivenMoreThan100MoviesAvailable {
        ...
        private val moviesPage = (1..60).map { Dtos.movieDto.copy(id = it) }

        @BeforeEach
        fun setUp() {
            coEvery { apiService.getTrendingMovies(any(), 1, any()) } returns
                TrendingMoviesResponseDto(results = moviesPage, page = 1, totalPages = 2)
            coEvery { apiService.getTrendingMovies(any(), 2, any()) } returns
                TrendingMoviesResponseDto(results = moviesPage, page = 2, totalPages = 2)
        }
    ...
    inner class GivenApiReturnsMovieDetail {
        ...
        @BeforeEach
        fun setUp() {
            coEvery { apiService.getMovieDetail(Dtos.movieDetailDto.id, any()) } returns Dtos.movieDetailDto
        }
