...
class TrendingMoviesStateReducersTest {
    ...
    @MockK
    lateinit var movieDomainToPresentationMapper: MovieDomainToPresentationMapper

    @MockK
    lateinit var genreDomainToPresentationMapper: GenreDomainToPresentationMapper

    @MockK
    lateinit var weekRangeLabelProvider: WeekRangeLabelProvider
    ...
    fun setUp() {
        ...
        stateReducers = TrendingMoviesStateReducers(
    ...
    inner class GivenMoviesGenresAndWeekLabelAreAvailable {
        ...
        fun setUp() {
            every { weekRangeLabelProvider.currentWeekRangeLabel(any()) } returns WEEK_LABEL
            every { movieDomainToPresentationMapper.map(PresentationMocks.Movies.action) } returns PresentationMocks.PresentationMovies.action
            every { movieDomainToPresentationMapper.map(PresentationMocks.Movies.comedy) } returns PresentationMocks.PresentationMovies.comedy
            every { genreDomainToPresentationMapper.map(PresentationMocks.Genres.action) } returns PresentationMocks.PresentationGenres.action
            every { genreDomainToPresentationMapper.map(PresentationMocks.Genres.comedy) } returns PresentationMocks.PresentationGenres.comedy
        }
        ...
        inner class WhenMoviesAndGenresFinishLoadingWithoutErrors {
            ...
            fun loadingIndicatorDisappears() {
                val result = initialState.reduceWith(
        ...
        inner class WhenMoviesAndGenresLoadButMoviesFailedToFetch {
            ...
            fun moviesLoadFailureErrorIsShown() {
                val result = TrendingMoviesState().reduceWith(
                    stateReducers.contentLoaded(
                        movies = PresentationMocks.Movies.all,
                        genres = PresentationMocks.Genres.all,
    ...
    inner class GivenMoviesAreAvailableForDisplay {
        ...
        inner class WhenUserClearsTheActiveGenreFilter {
            ...
            fun noGenreFilterIsActive() {
                ...
                val result = stateWithGenre.reduceWith(
                    stateReducers.filterByGenre(
                        genreId = null,
                        movies = PresentationMocks.Movies.all
                    )
    ...
    inner class GivenScreenHasAPreviousLoadError {
        ...
        private val stateWithErrors = TrendingMoviesState(
            isLoading = false,
    ...
    inner class GivenScreenIsInALoadingState {
        ...
        inner class WhenMoviesFailToLoad {
            ...
            @DisplayName("THEN the loading indicator disappears")
            fun loadingIndicatorDisappears() {
                assertFalse(loadingState.reduceWith(stateReducers.loadFailed(listOf(MovieError.MOVIES_LOAD_FAILED))).isLoading)
            }
            ...
            fun moviesLoadFailureErrorIsShown() {
                assertEquals(
                    ...
                    loadingState.reduceWith(stateReducers.loadFailed(listOf(MovieError.MOVIES_LOAD_FAILED))).errors,
                )
            }
        }
    }
    ...
    inner class GivenScreenIsInItsInitialState {
        ...
        inner class WhenUserOpensTheSortOptionsSheet {
            ...
            fun sortSheetBecomesVisible() {
                assertTrue(TrendingMoviesState().reduceWith(stateReducers.sortSheetVisible(true)).showSortSheet)
            }
        }
    }
