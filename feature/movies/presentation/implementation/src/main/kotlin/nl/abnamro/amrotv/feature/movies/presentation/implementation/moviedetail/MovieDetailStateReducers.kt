...
class MovieDetailStateReducers @Inject constructor(
    ...
    fun loading(): StateReducer<MovieDetailState> =
        StateReducer { it.copy(isLoading = true, errors = persistentListOf()) }

    fun detailLoaded(movieDetail: MovieDetail): StateReducer<MovieDetailState> =
        StateReducer {
            it.copy(
                isLoading = false,
    ...
    fun loadFailed(errors: List<MovieError>): StateReducer<MovieDetailState> =
