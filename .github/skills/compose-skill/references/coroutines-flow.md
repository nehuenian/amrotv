...
| `Dispatchers.IO` | Network, database, file I/O (Android/JVM only) | Android/JVM only |
| `Dispatchers.Default` | Background work in `commonMain` (network, database, parsing, computation) | All targets |

> **Project rule:** In this codebase, `commonMain` code must **not** use `Dispatchers.IO` — it is not guaranteed on all KMP targets. Use `Dispatchers.Default` (or inject `AppCoroutineDispatchers` from `core/base`) for all background work in shared code.
...
class ProductRepository(
    ...
    private val dispatchers: AppCoroutineDispatchers, // injected from core/base
) {
